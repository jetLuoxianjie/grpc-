...
func (s *SpiderServer)GetXiaohua(ctx context.Context, in *proto.Req) ( res *proto.Res,err error) {

   res=new(proto.Res)
	start,err:=strconv.Atoi(in.Start)
	end, err:=strconv.Atoi(in.End)
	toWork(start,end)
...
func SaveJoke2File(idx int, fileTitle, fileContent []string)  {
	...
	path := "D:/1/"+s+"第"+ strconv.Itoa(idx) + "页.txt"
